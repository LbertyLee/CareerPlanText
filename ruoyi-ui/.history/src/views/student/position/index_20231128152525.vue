  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="岗位名称" prop="positionName">
        <el-input v-model="Name" placeholder="请输入岗位名称" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleRowClick(null)">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini"
          @click="dialogTableVisible = true">设置主目标</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="positionList">
      ...
      <el-table-column label="总体完成率" align="center" prop="completionRate" />
      <el-table-column label="本年度1-6月项目数" align="center" prop="completedQuantity1" />
      <el-table-column label="本年度1-6月完成率" align="center" prop="completionRate1" />
      <el-table-column label="本年度7-12月项目数" align="center" prop="completedQuantity2" />
      <el-table-column label="本年度7-12月完成率" align="center" prop="completionRate2" />
      <el-table-column label="状态" align="center">
        <template slot-scope="scope">
          <el-tag :type="scope.row.state === 1
              ? 'primary'
              : scope.row.state === 2
                ? 'success'
                : 'danger'
    ...
    : scope.row.state === 2
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <!-- 发布和草稿状态 -->
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleRowClick(scope.row)"
            v-if="scope.row.state != 0">编辑</el-button><!-- 只给发布状态用 -->
          <el-button size="mini" type="text" icon="el-icon-chat-dot-round" v-if="scope.row.state == 1"
            @click="handleComment(scope.row, 3)">目标自评</el-button>
          <!-- 只给发布状态用 -->
          <el-button size="mini" type="text" icon="el-icon-delete" v-if="scope.row.state === 1"
            @click="ToStop(scope.row)">废止</el-button>
          <el-button v-if="scope.row.state === 2" type="text" icon="el-icon-folder-add"
            @click="PublicRowClick(scope.row)">发布</el-button>
          <el-button v-if="scope.row.state === 2" icon="el-icon-scissors" size="mini" type="text"
            @click="DeleteRowClick(scope.row)">删除</el-button>
          <!-- 只给废止状态用 -->
          <el-button size="mini" type="text" icon="el-icon-view" v-if="scope.row.state === 0"
            @click="SeeObject(scope.row)">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="first">
      <el-dialog padding="0" :visible.sync="ObjectVisible" width="100%">
        <addposition :positionDetail="positionDetail" :ComputedisMain="ComputedisMain" :Computedstate="Computedstate"
          :ObjectVisible="ObjectVisible" @getIndex="getIndex" @closeDialog="handleDialog" />
      </el-dialog>
    </div>

    <!-- 设置主目标弹出层 -->
    <el-dialog title="设置主目标" :visible.sync="dialogTableVisible" width="25%" class="SetObject" @close="positionId = ''">
      <el-select v-model="positionId" placeholder="请选择">
        <el-option v-for="item in ObjectList" :key="item.positionId" :label="item.positionName" :value="item.positionId">
        </el-option>
      </el-select>
